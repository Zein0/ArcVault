...
**3. Context-aware escalation via LLM.** The current escalation triggers are purely mechanical — keyword match, confidence threshold, dollar regex. They can't reason about context. A message like "it's been slow for a few users this morning" has no keywords and no dollar amount, but in context it might warrant escalation. I'd add an explicit escalation-assessment step where the LLM reads the full message and answers a single yes/no question: "Does this message describe a situation that could affect multiple users, involve data risk, or require immediate action?" This runs after classification, only adds one LLM call, and catches cases the deterministic rules miss. The deterministic rules stay as a hard floor — the LLM adds a soft ceiling.

**4. Semantic escalation detection.** The current keyword list is exact-match only — it misses typos ("ouatge"), synonyms ("all accounts down"), and paraphrases ("nobody can log in"). I'd replace the hardcoded array with a small embedding-based similarity check: encode the message and compare it against a set of canonical escalation phrases using cosine similarity. This catches the intent, not just the literal string.
